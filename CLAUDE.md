...
- **일조권 사선제한**: 건축법 제61조 제1항 (2026.8.11 개정 · 2026.11.12 시행 — 이격 기준이 시행령 86조에서 법률 본문으로 승격, 3단계)
  - 10m 이하 부분: 인접대지경계선에서 1.5m 이상 이격
  - 10m 초과 17m 이하 부분: 5m 이상 이격 (신설 · 고정값)
  - 17m 초과 부분: 해당 높이의 1/2 이상 이격
  - 개정 전(시행령 86조①, ~2026.11.11): 10m 이하 1.5m / 초과 h/2 — 시뮬레이터 [개정 전 보기]로 비교 가능
...
- **2026-09-18** — **규모·주차·비용·사업성·PDF 단일 계산원 + 미확인 표시** (브랜치 `fix/scale-consistency`, 미배포 · 외부 검토 보고서: 역삼동 825-3).
  ...
  - **store**: `lotSqm`(정밀 원본)·`lotAreaSource`·`officialLotSqm`·`floor1HeightM`·`typicalFloorHeightM`·`permitDate`·`roadMSource`. 비용 store `linked`(지상·지하·주차 대수 규모검토 연동, 수동 변경 시 끊김+되돌리기). 사업성 store `sources`(값 출처 — 기본값이면 판정 보류).
  - **법령(법제처 MCP 원문 2026-09-18)**: 건축법 시행령 119조①3호 다목(필로티 바닥면적 불산입 요건)·①4호 가·나목(용적률 산정 시 지하층·지상 부속주차 제외 — 벽체식도 제외, 기존 "벽체식 연면적 산입" 표기 오류). 일조 규칙 기본값은 기준일(신청 예정일, 없으면 검토일) 기준 — 2026.11.12 이후 신청분부터 개정 후(부칙), 그 전은 개정 전.
  - **PDF**: 표지·요약에 미확인 사항 박스, 결론 문장 비확정형, "토지비 포함 총사업비" / "건축·부대비 소계" 분리, 일조 미적용 필지는 단면도·후퇴 문구 숨김, 섹션 번호 자동(1..N), 층별표는 동일층 묶음(1F~13F × 13개 층)으로 한 쪽, 제목 고아 방지(첫 블록과 wrap=false), 검수 상태·AI 실행 여부 표기, LTV→LTC, IRR·마진·이자·세금 정의 수록. ⚠ react-pdf `fixed` 표 머리글은 다음 쪽 첫 행과 겹침 — 쓰지 말 것.
  - **검증**: vitest 38개(`npm test`, 면적 왕복·층수/높이·주차 경계·지하/비용 연결·일조 기준일·판정 보류·미확인 규제) / tsc 0 / eslint 신규 0 / next build ✓ / 로컬 실조회(역삼동 825-3) 화면 KPI·비용·사업성 일치 / Node 렌더 PDF(업무·일반상업 10쪽, 주거 9쪽) 텍스트·이미지 확인. ⚠ 브라우저 패널이 숨김 상태라 3D 캔버스 미초기화 — 3D 화면·캡처는 미검증.
  - **미리보기 검증(2026-09-18, 커밋 840d74f)**: 브랜치 push → Vercel 미리보기(target=preview) 자동 생성. 미리보기에서 역삼동 조건 수동 입력 → KPI 동일, 3D 전체 매스 표시, 브라우저 [분석 없이 PDF만]→[PDF 다운로드] 경로로 10쪽 PDF 생성(워커, 약 2분) 확인. 이 과정에서 두 가지 추가 수정: ① 3D 카메라가 세로형(좁은) 화면에서 상부를 자르던 것 → 화면비 보정 ② PDF 캡처가 4:3 중앙 크롭 + 목표 높이 14m 상한 때문에 고층 상부가 잘리고 투명 배경이 검정 → 크롭 시야 기준 거리·목표 0.45h·옅은 하늘색 배경, 보고서 3D 이미지 objectFit contain. ⚠ 미리보기는 로그인 없이 지번 조회 불가라 조회 경로는 로컬에서만 확인.
  - 역삼동 825-3 전후: 대지 393.39→394.8㎡ · 건축면적 236.03→236.88㎡ · 연면적 3,147.11→3,158.4㎡ · 13.3층/46.7m→지상 14층(최상층 부분층 33%)/49.0m · 주차 31→32대(31.58, 비고 6) · 주차면적 775/930→960㎡ · 지하 0→960㎡(5개 층) · 건축·부대비 91.71→132.42억 · 사업성 "양호"→"판정 보류".

- **2026-09-09** — **3D 도로를 실제 지적 도로 필지 모양으로** (운영자: "도로 위치좀 봐줘 이상해"). 전에는 대지 외곽상자 남쪽에 직사각형 도로를 고정으로 붙여 회전·비정형 실형상에서 옆 건물 위에 떠 보였다. `fetchVworldRoadParcels`(연속지적도 지목 '도' 필지, geometry=true) + `/api/vworld?kind=roadpolys` → `RoadSurfaces`(ShapeGeometry, y=0.02, 라벨은 대지에 가장 가까운 조각 무게중심). 도로 필지를 받으면 남쪽 합성 도로·인도·중앙선은 숨김(박스 모드·조회 실패 시만 표시). 검증: 역삼동 825-3 캡처(capture3D)에서 실제 도로선과 일치.
...
- **2026-09-05** — **☀️ 아파트 일조 보기 `/sunlight`** (운영자: "아파트를 검색해서 아파트단지 햇빛이 드는 것을 시간대별로 보여줘"). 크레딧 차감 없음, 게이트 없음.
  ...
  - 🏢 동 선택 상세(표·3D·지도 클릭): `computeBuildingDetail` — 면(변)별 방향(8방위)×1층/중간층/최상층 타임라인(25칸)·그림자 원인 동·계절 비교. PDF 3장으로 실림(`detail`). 보고서 하단 표기(사무소명·담당)는 localStorage `gyumo_sunlight_office` → PDF 푸터·면책 작성 줄 치환.
  - 🗺️ 지도 엔진: 카카오맵 우선(`lib/kakaoMap.ts` probeKakao, mrk-realestate 와 같은 로더) → 실패(키 없음·도메인 미등록·한도·크롤러) 시 VWorld leaflet. `SunMapAuto` 가 고른다. **NEXT_PUBLIC_KAKAO_JS_KEY**(Vercel Config, Production·Preview 2026-09-05 등록, mrk 와 같은 카카오 앱 JS 키) — 카카오 콘솔 Web 도메인에 `https://gyumo.vercel.app` 등록돼야 운영에서 카카오맵이 뜬다(미등록이면 조용히 VWorld 폴백).
...
- **2026-09-04 (2)** — **💡 Tip: 위반건축물 이행강제금 계산기** (운영자: "이행강제금 계산하는 것… Tip으로 하나 넣자").
  - **법령(법제처 MCP 원문)**: 법 80조①1호(1㎡ 시가표준액×50%×위반면적×영 115조의3① 비율 — 건폐율 80/용적률 90/무허가 100/무신고 70, 조례로 낮춰도 60% 이상)·2호(별표15: 대수선·용도변경·조경·건축선·구조·피난·내화·방화지구·마감·높이·일조·설비 10%, 미사용승인 2%, 기타 3%)·단서(60㎡ 이하 주거 + 영 115조의2① 유형 주거 → ½ 범위 조례). 가중 80조②/영 115조의3②(영리 용도변경·신증축 >50㎡, 영리 세대 5↑, 3년 2회) 현행 100% 범위 → **2027.2.12 시행(법률 21880호) 50~100% 의무 + 반복부과 의무 + 다음 연도 가중**. 감경 80조의2/영 115조의4(소유권변경·임차인·30㎡·집합 5㎡·사용승인 당시 75%, 단서 해당 시 50%, 농어업 1/5, 1992 이전 주거 80/60%) — 가중 대상은 감경 제외.
  - `lib/calc/enforcementFine.ts`(순수·무의존, node 10케이스 검산) + `EnforcementFineSheet.tsx`(② 탭 법령근거 아래 팁 카드 + 우측 시트: 유형·시가표준액·면적·조례비율·주거특례·가중(현행/2027 토글)·감경·연 횟수·연수 → 1회/연간/누적 + 단계별 산식·근거·경고). 시뮬레이터 건폐율·용적률이 상한 초과면 팁에 초과면적 표시 + 시트 열 때 유형·면적 프리셋. 건축물대장 시가표준액 조회값 있으면 [적용] 버튼.
  - 검증: tsc 0 / eslint 0 / 브라우저 DOM — 무허가 50㎡×80만 → 2,000만, 미사용승인 3억 주거 → 300만, 용적률 320% 프리셋 793㎡.
  - **📄 계산결과 보고서 PDF**(운영자 추가 요청): `components/report/EnforcementFineDocument.tsx` — A4 1쪽(브랜드 띠·KPI 3·입력 조건표·산정 과정표·유의사항·근거 법령·절차·면책). 시트 결과 아래 [계산결과 보고서 PDF 다운로드] → `pdf().toBlob()` 즉시 생성·해제(ReportDialog 패턴). 실측 38KB·1쪽, 용적률 초과 793.4㎡+3년내 2회 가중 → 1회 5.71억·3년 17.14억 확인.

- **2026-09-04** — **일조 높이제한 2026.8.11 건축법 개정 반영 (개정 후 원칙 + 개정 전 비교) + 보고서 여백 정리** (운영자: "일조제한 바뀌었어… 개정전 클릭하면 비교해서 무엇이 좋아졌는지 확인").
  - **법령 확인(법제처 MCP)**: 건축법 제61조① 개정(2026.8.11 공포 제21880호, **2026.11.12 시행**) — 이격 기준이 시행령 86조①에서 법률 본문으로 올라오며 **3단계**: 10m 이하 1.5m / **10m 초과 17m 이하 5m 고정(신설)** / 17m 초과 h/2. 정북 인접대지가 전용·일반주거 아니면 제외 규정도 법 단서로 승격. 두 규칙은 17m에서 만나(8.5m) 차이는 10~17m 구간(층고 3.5m 기준 3F·4F)뿐이며 개정 후가 불리해지는 높이는 없음.
  - **단일 출처**: `lib/calc/sunlight.ts` — `SunlightRule("revised"|"legacy")`, `requiredSetbackM(h, rule)`(절대 이격)·`extraSetbackM`(1.5 대비 추가)·`envelopeProfile(topH, rule)`(2D/3D 사선면 꼭짓점)·`compareRulesByFloor`·`SUNLIGHT_RULE_META`. 2D 입면·3D 박스/실형상 envelope·floorTable·shadowCheck·보고서가 전부 이 함수만 사용(기존엔 `fH/2-1.5` 같은 식이 8곳에 흩어져 있었고 2D 사선과 층 후퇴가 서로 안 맞았음 — 이번에 정합).
  - **store** `sunlightRule`(기본 revised) + `ControlPanel` `SunlightRulePicker`: [개정 후(원칙) | 개정 전 보기] 세그먼트 + 3단 요약. 개정 전 선택 시 시뮬레이터 전체(2D·3D·KPI·PDF)가 개정 전으로 바뀌고 **비교 카드**(개정 전/후 실제 연면적·차이·층별 이격표·완화 문구) 표시. 실측(200평·60%·200%·3.3층): 개정 전 1,211㎡ → 후 1,229㎡(+18㎡), 3F 5.25→5.00 / 4F 7.00→5.00.
  - **보고서**: `ReportInputs.scale.sunlightRule`·`sunlightCompare`(buildInput이 반대 규칙으로 한 번 더 계산). `SunlightDiagram`을 규칙별 실제 계단으로 다시 그림(개정 후일 때 개정 전 사선을 회색 점선으로 병기), `(c-1) 일조 규칙 개정 전·후 비교` 블록(3 KPI + 층별표) 신설, 법규 검토표·부록 법령·AI 프롬프트에 규칙 명시. 학습시트 2번 섹션 신설(개정표·수혜 규모·조례 위임·시행일 안내). `LegalBasis`·CLAUDE.md §5 현행화.
  - **PDF 여백**: 공백 원인은 h2 `minPresenceAhead={300}`·h3 `{130}`(다음 내용 105mm/46mm 확보 못 하면 섹션 통째로 다음 장) → 110/60으로 완화, 3D 블록(기본뷰+남북 2컷 ≈330pt 한 덩어리)을 두 wrap 블록으로 분리, section 여백 26→18. 
  - ⚠ 시행일 전(2026.11.11까지) 접수 인허가는 개정 전 기준 — UI·시트·보고서에 명시. 조례가 더 큰 거리를 정할 수 있어 관할 확인 문구 유지.
  - 검증: tsc 0 / eslint 신규 0(3D 2141 set-state-in-effect는 다른 세션의 이웃건물 fetch, ReportDocument 미사용 변수 3건 기존) / next build ✓ / 순수함수 스모크(3.5~21m 8개 높이·프로필·GFA) / 브라우저 — 픽커·비교카드·2D 사선·3D 라벨 "일조 · 10m↓1.5m / 17m↓5m / 초과 h½" / PDF pdf.js 렌더로 페이지 여백 확인.
...
- **2026-08-17** — **고도화 1~3순위: 전국 조례 자동 적용 + 규제 체크리스트 카드 + 승강기 검증** (운영자 요청: "1순위부터 3순위 순서대로").
  - **[1순위] 전국 지자체 도시계획조례 자동 적용**: 그동안 서울 조례값을 전국에 적용 — 지방 땅이 서울 기준으로 검토돼 규모가 틀리게 나왔음. `lib/data/ordinance-density.json`(mrk-realestate/공법규제분석 앱, 대표 감수, 전국 184개 시·도/시·군·구) 채택 — 건축이야기 조례 데이터는 "자동추출-검증필요" 상태라 미채택. `lib/ordinance-db.ts`가 법정동코드로 지자체 조례를 찾아 store에 override(서울은 zones.ts 정본 유지, CBD 특례 보존). ControlPanel에 "○○ 도시계획조례 자동 적용" 배지+원문링크. ⚠ 184개 중 광역시급 9곳만 조문 링크 정확, 나머지 175곳은 수치만 검증됨(링크는 검색 안내로 대체) — 정직하게 명시.
  - **[2순위] 규제 체크리스트 카드**: 지번 조회 직후 추가 API 호출 없이 "이 땅의 걸림돌"을 즉시 요약. `lib/regulation-checklist.ts` — 토지이용계획 위험 키워드(개발제한구역 등 danger/농업진흥구역 등 warning) + 농지·산지 부담금 간이 추정(1순위 이전 법령정합 작업의 현행 공식 재사용) + 정북 일조권 대상 안내. `RegulationChecklist` 컴포넌트, LandLookup ① 탭 최상단 삽입.
  - **[3순위] 가설계 법정 승강기 검증**: 세대수만 뽑고 승강기 자리가 있는지 확인 안 하던 것을 보완. `lib/calc/elevator.ts` — 건축법 64조 + 「건축물의 설비기준 등에 관한 규칙」 별표1의2(법제처 원문 확인) 그대로 구현, 15개 용도를 3개 그룹에 매핑. `calculateSchematic`에 usage 파라미터 추가 → 법정 대수×개략면적이 코어 면적에 들어가는지 검증, SchematicPlanner에 충족/부족 카드.
  - 검증: 3개 기능 모두 tsc 0/eslint 0/build 0 + 순수 함수 노드 스크립트 검증(1순위 9항목/2순위 9항목/3순위 12항목) + 브라우저 임시 렌더 확인 후 정리. 프로덕션 배포 5건 전부 READY, 주요 페이지 200 확인.

- **2026-08-16** — **시뮬레이터 법령 정합 전수검증 — 현행 법령 대조 6건 교정** (운영자 요청: "값이 법령에 어긋난 것 없는지 하나씩 확인").
  - 법제처 MCP로 현행 원문 대조: 건축법 시행령 86조 / 주차장법 시행령 별표1 / 서울 주차 조례(2026.7.13) 20조·별표2 / 농지법 시행령 53조·시행규칙 47조의2 / 산림청 고시 제2026-16호 / 개발이익환수법 13조 / 국토계획법 시행령 84조.
  - **교정 6건**: ① 정북 일조는 전용·일반주거만 — 준주거 적용 제거(`Zone.sunlight` 플래그 분리, 규모 과소 해소) ② 공장 350㎡당(별표1 7호)·창고 400㎡당(8호) 분리 + 위락(1호 100㎡당) 신설 ③ 서울 조례 실값 공장 233·창고 267 (지역 DB class100~400 재편, 부산·인천·대구 세분값은 시행령 폴백 명시) ④ 다가구 = 주택건설기준 27조 준용(세대 기준)으로 전환 ⑤ 주차 끝수는 0.5 반올림·1대 미만 0대(비고 6호) — `legalRoundSpaces` (기존 무조건 올림 과대) ⑥ 농지부담금 진흥지역 안 30%/밖 20% 차등 + 산지 공시지가 가산 0.1%(기존 1% — 10배 과대)·상한 8,340원·보전 배율은 기본단가에만.
  - UI: 농지 안/밖 프리셋 칩, 산지구분 3단가 프리셋(준보전 8,340/보전 10,840/제한 16,680 — 2026 고시), 준주거 일조 자동 OFF 안내, 학습시트 문구 현행화.
  - **수기 검산 30항목 통과** (scratchpad smoke.ts — 일조 층별 합계, 주차 반올림 경계 10.1/10.5/0.67, 부담금 상한 캡, 41.06억 회귀).
  - ⚠ 남은 확인: 부산·인천·대구 조례 공장/창고 세분값 원문 재확인(현재 시행령 350/400 폴백), 2026 고시 보전산지 단가 10,840원은 배율(130%) 파생값 — 고시 원표(이미지) 대조 권장.
...
- **2026-07-18 (4)** — **등급 토글 + 등급 저장 버그 수정 + 가입 알림·시트 연동 + 반응형 깨짐 수정**.
  ...
  - **가입 알림**: `lib/notify/email.ts`(Resend HTTP 직접 호출, 패키지 없음 — 키 미설정·실패를 전부 삼켜 가입을 막지 않음) + `lib/notify/new-signup.ts`(`gyumo_profiles.signup_notified_at`으로 중복 방지, **먼저 도장 찍고 발송**해 동시 요청 이중 발송 차단). 이메일 가입·구글 콜백 양쪽 연결. 관리자 대시보드에 최근 7일 신규 가입 배지.
  - **구글 시트 연동**: `/api/sheet/members?token=` CSV(IMPORTDATA 전용, 시트는 세션 불가라 토큰 인증, `no-store`). 프로덕션 검증 — 토큰 없음·오답 401, 정답 시 8열 CSV 정상.
  - **⚠ Resend 제약**: 도메인 인증 전에는 **계정 주소(ko37258@gmail.com)로만 발송 가능** → `scko@mrhomes.co.kr` 수신 403. `ADMIN_NOTIFY_EMAIL`을 ko37258@gmail.com으로 설정. 회사 도메인 수신을 원하면 resend.com/domains에서 mrhomes.co.kr 인증 필요. 발송 키는 `Sending access` 권한이라 `GET /domains`는 401(정상).
  - **환경변수 등록 완료**(Vercel CLI, Production): `SHEET_TOKEN`·`RESEND_API_KEY`·`ADMIN_NOTIFY_EMAIL`.
  - **🐛 반응형 깨짐 2건**: ① 마케팅 Nav가 375px에서 항목이 눌려 **글자가 세로로 한 자씩** 쪼개짐 → `whitespace-nowrap`+`shrink-0`, 모바일 2행(브랜드/메뉴)+메뉴만 가로 스크롤, 여백 축소, CTA 라벨 단축(모바일 "시작하기"). ② 건축이야기가 **761~980px 구간에 CSS 규칙이 없어** 문서폭 954px로 가로 스크롤 발생(원본 CSS는 ≤760·≥980만 처리) → 해당 구간만 보정. 검증: 8개 페이지 × 360/768/1280px + 건축이야기 8개 폭 — 가로스크롤·글자잘림·세로쪼개짐 0건.
...
- **2026-07-18 (2)** — **건축이야기(MR.K의 건축규제 이해하기)를 gyumo 내부로 편입** (운영자 요청: "별도 사이트로 하지 말고 gyumo 안에서 돌게").
  ...
  - **구현**: `next.config.ts` rewrite `/building-law` → `/building-law/index.html`. **⚠ 함정**: 끝 슬래시 없는 `/building-law`에서 상대경로가 루트로 풀려 css/js/데이터 전부 404 → **index.html 자산 경로를 `/building-law/...` 절대경로로 고정**해 해결. 상단에 gyumo 복귀 띠(`.gyumo-backbar`, 테마 무관하게 네이비 #020425 + 골드 #FFCF0D 고정) 추가. Nav "건축이야기" 외부 링크 → `/building-law`.
  - 검증: tsc 0 / eslint 0 / build 33 라우트 / **프로덕션(`edacc75`) — ORDINANCE_DATA 로드(용도지역 21·시도 10), 경기 선택 시 결과 10,108자 출력, 용도 29종·개념 6종 렌더, 복귀 띠 클릭 → `/` 이동, 콘솔 에러 0.**
  - ⚠ 남은 건: ① 원본의 히어로 이미지 `assets/mrk-interview.jpg`가 **넷리파이 원본에서 이미 404**(클론 문제 아님) — 파일 확보 시 `public/building-law/assets/`에 투입 ② 같은 콘텐츠가 넷리파이·gyumo 2곳에 존재 → **넷리파이를 `gyumo.vercel.app/building-law`로 리다이렉트해 원본 일원화 권고**(운영자 결정 대기).

- **2026-07-18** — **랜딩·요금제를 현행 크레딧 제도로 재작성 + 입금 계좌 확정** (운영자 요청: "현재 우리 제도로 다시 설명하는 칸 + 내용 고도화 + UI 도 AI 스타일 안 나게").
  - **입금 계좌 확정**: `lib/credits.ts` BANK_INFO = **카카오뱅크 3333-37-3784694 / 예금주 고상철**. (운영자 조치 3건 중 마지막 항목 완료 — 환경변수 `NEXT_PUBLIC_BANK_*` 미설정 시 이 값 사용.)
  - **랜딩(`app/(marketing)/page.tsx`) 전면 재작성**: 없어진 제도(일 3건 무료 / Pro 월 19,900 / Business 월 99,000 / 가맹점·멘토스쿨 특별플랜)를 설명하던 PRICING TEASER·FINAL CTA 문구를 폐기. 신설 — ① **크레딧 제도 섹션**(네이비 풀블리드: 가입 3크레딧 / 10회 1만원 12크레딧 / 30회 2.5만원 36크레딧(골드 강조) + 송금→신청→3시간 내 지급→정회원 전환 4단계 + 승인 후 2개월 유효 안내 + 계좌 표시) ② **기능 소개를 현재 4단계 도구**(토지가치분석·규모검토·비용·사업성)로 갱신 — 조회/계산/검증 3등분 카드 → 번호(01~04) 매긴 세로 스텝 + 태그 칩. 헤드라인 "지번 한 줄로 땅값부터 사업성까지", CTA는 `/signup`(무료 3회)로.
  - **요금제(`pricing/page.tsx`) 재작성**: Free/Pro/Business + 토스 정기결제·환불 FAQ 폐기 → 크레딧 3종 + 충전 절차 4단계 + FAQ 6종(차감 시점·재조회 차감·만료 우선순위·계좌이체 한정·법령 근거·감정평가 아님).
  - **AI 티 제거**: SCROLL 힌트, `# SCALE REVIEW ·` 섹션 아이브로, ⚖️ 등 이모지 장식, 3등분 균등 카드 제거. 레이아웃 계열을 섹션마다 다르게(히어로 / 세로 스텝 / 네이비 타일+절차 / 정의목록 split / CTA). 브랜드 네이비 #020425 + 골드 #FFCF0D와 자동차 애니메이션은 유지.
  - `privacy/page.tsx` 기존 react/no-unescaped-entities 오류 2건 동시 수정.
  - 검증: tsc 0 / eslint 0 / `next build` 0 오류 / 로컬 렌더 확인 / **프로덕션(`3e4ba7c` → READY) 검증 — 계좌·크레딧 섹션·무료 3회 CTA 노출, "일 3건"·"19,900"·"SCROLL" 전부 사라짐 확인**.
  - ⚠ 남은 확인: 정회원 신청 → 관리자 승인 → 크레딧 지급·정회원 승격 E2E(운영자가 신청, 관리자 화면에서 승인 확인 필요).
...
- **2026-07-17 (14)** — **용도별 분양가·임대료 팝업 + 보고서 수록 항목 체크 + 프로젝트 이력** (운영자 피드백 3종).
  ...
  - **보고서 수록 항목 선택**: ReportDialog에 📑 체크 6종(토지 정보·용도별 시세표·비용 페이지·사업성·주변 시세·3D 캡쳐) — 미조회 항목 자동 비활성+사유 표기, 해제 시 PDF·AI 프롬프트에서 제외(`applySections`, `includeCostPage`). ReportDocument에 UsePricesPage(선택 수록) + CostPage 게이트.
  - **📁 프로젝트 이력**: store/history(zustand persist, LocalStorage `gyumo_project_history`, 최대 100건) — 조회 성공 시 자동 기록(주소·면적·용도지역·추정가·일시), ① 탭 하단 카드에서 최근순/지번순 정렬·클릭 재조회(딥링크 ?address=)·개별/전체 삭제.
  - 프로덕션 E2E(역삼동 825-3): 팝업 표 — 아파트 7,402만/평(356건)→공급전환 5,774만, 다가구 월세 8만/평(1,430건), 상가 1층 6,128만/평 등 전 행 표시 ✓ / 이력 카드 1건 기록 ✓ / 보고서 체크박스 — 조회된 항목(토지·용도별) 활성, 미조회(사업성·주변시세) 비활성 ✓.

- **2026-07-17 (13)** — **신축 시세 실거래 사례 목록 + 테마 드롭다운 지도 가림 수정** (운영자 피드백: "플렉시티처럼 실제 거래금액을 보여줘야 이해가 됨" + "잘리는 현상").
  - `/api/newbuild-price`: 집계(중앙값)에 더해 **실제 거래 사례 최신순 10건씩** — `residential.tradeSamples`(단지명 mhouseNm·계약연월·층·전용㎡·거래가·㎡당·준공연도), `jeonseSamples`(보증금), `commercial.samples`(층·용도·건물㎡). LandLookup 신축 시세 카드에 [매매]/[전세]/[상가] 배지 행(기본 매매3·전세2·상가2 → [N건 더보기 ∨] 시 각 10건).
  - **ThemeQuickToggle 드롭다운 z-50 → z-[1200]**: ① 탭 신설로 지도(leaflet pane 400·컨트롤 1000)가 헤더 바로 아래에 오면서 드롭다운이 지도에 덮여 잘리던 현상 해결.
  - 프로덕션 E2E(역삼동 825-3): 사례 행 "매매 2026.06 · S클레스4차 3층 · 29.86㎡ (2023년식) — 7.20억 (2,411만)" 등 + [23건 더보기] → 30행 전체 펼침·접기 확인. 드롭다운 elementFromPoint 검사 — 지도 위에서 5개 테마 전부 노출·비가림 확인.
...
- **2026-07-17 (9)** — **플렉시티 벤치마킹 2종: 토지가격 보드 완성형 + ⑥ 가설계(기획설계 개요)**.
  ...
  - **[B] ⑥ 가설계 신설** (플렉시티 기획설계 v1): `lib/calc/schematic.ts`(공급면적=전용/전용률, 층당=⌊기준층/공급⌋, 총=층당×주거층수(필로티 시 −1)) + `SchematicPlanner.tsx` — 유닛 프리셋(29/49/59/84㎡)+전용면적·전용률 SliderInputPair → 4-KPI(총세대/공급면적/전용합/코어) + **기준층 개략 배치도 SVG**(판상형 중앙코어, 유닛 분할+북측 표기) + **[⑤ 주차 산정에 N세대 자동 반영]**(tieredHousehold 용도로 전환+전용면적 티어에 배정, 비티어 용도면 다세대연립로 전환). 주거계 5용도에서 활성, 비주거는 안내문. page.tsx ⑤와 ⑥ 사이 삽입.
  - 검증: 로컬 — 다세대연립·200평·건폐60·용적200·전용59·전용률78 → 층당 5×3개층=15세대(수기 일치), 주차 반영 15×0.8=12대 ✓ / 프로덕션 — 성내동 562 보드(추정 53.4억/공시 12.2억/건물 3.0억+배수 4.6·4.1배) + ⑥ 렌더 ✓.
  - base-ui Select 자동화 팁: 합성 pointer 이벤트 불안정 — **keydown(ArrowDown→Home→ArrowDown×n→Enter) 키보드 시퀀스가 안정적**.
